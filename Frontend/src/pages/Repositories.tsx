...
const Repositories: React.FC = () => {
  ...
  const [scanSource, setScanSource] = useState<"github" | "gitlab" | "url">(
    "github",
  );
  ...
  const [scheduleModalRepo, setScheduleModalRepo] = useState<GitRepoDto | null>(
    null,
  );
  ...
  const [scheduleBranch, setScheduleBranch] = useState("main");
  const [scheduleScanMode, setScheduleScanMode] = useState("auto");
  const [scheduleTargetDomain, setScheduleTargetDomain] = useState("");
  const [scheduleDastUrl, setScheduleDastUrl] = useState("");
  ...
  const selectedScan =
    scanTypes.find((s) => s.value === scanType) || scanTypes[0];
  ...
  useEffect(() => {
    ...
    const loadRepos = async () => {
      ...
      setReposError("");
      const [githubResult, gitlabResult, scannedResult] =
      ...
      setScanSource((current) => {
        ...
        if (
          ...
          githubResult.value.data.length > 0
        ) {
          return "github";
        }
        if (
          gitlabResult.status === "fulfilled" &&
          gitlabResult.value.data.length > 0
        ) {
          return "gitlab";
        }
  ...
  // Load scheduled scan summary (badge display)
  useEffect(() => {
    getScheduledSummary()
  ...
  const handleOpenScheduleModal = async (
    ...
    e.stopPropagation();
    const assignedRepo = scannedRepositories.find(
      (r) => r.repoUrl === repo.htmlUrl,
    );
    ...
    setScheduleBranch(assignedRepo?.branch || "main");
    setScheduleScanMode(assignedRepo?.scanMode || "auto");
    setScheduleTargetDomain(assignedRepo?.targetDomain || "");
    setScheduleDastUrl("");
    setScheduleError("");
    setScheduleSuccess("");
    ...
    if (assignedRepo?.id) {
      getRepositoryScheduledScans(assignedRepo.id)
  ...
  const handleCreateSchedule = async () => {
    if (!scheduleModalRepo) return;
    const assignedRepo = scannedRepositories.find(
      (r) => r.repoUrl === scheduleModalRepo.htmlUrl,
    );
    if (!assignedRepo?.id) {
      setScheduleError(
        "Ce repository doit être scanné au moins une fois avant de planifier.",
      );
      return;
    }
    ...
    try {
      await createScheduledScan({
        repositoryId: assignedRepo.id,
        repositoryName: scheduleModalRepo.name,
        repoUrl: scheduleModalRepo.htmlUrl,
        branch: scheduleBranch || "main",
        scanMode: scheduleScanMode || "auto",
        targetDomain: scheduleTargetDomain || undefined,
        dastTargetUrl: scheduleDastUrl || undefined,
  ...
  const handlePauseSchedule = async (id: number) => {
    ...
    if (scheduleModalRepo) {
      const r = scannedRepositories.find(
        (x) => x.repoUrl === scheduleModalRepo.htmlUrl,
      );
      if (r?.id)
        getRepositoryScheduledScans(r.id)
  ...
  const handleQuickScan = async (repo: GitRepoDto) => {
    ...
    try {
      ...
      setLogs((prev) => [
        ...prev,
        parseLogLine("[ERROR] Failed to start scan."),
      ]);
      setScanning(false);
    }
  ...
  const githubFilteredRepos = filteredRepos.filter(
    (r) => r.provider === "GITHUB",
  );
  const gitlabFilteredRepos = filteredRepos.filter(
    (r) => r.provider === "GITLAB",
  );
  ...
  const activeRepoCount = isGitlabView
    ? gitlabFilteredRepos.length
    : githubFilteredRepos.length;
  const activeRepos = isGitlabView ? gitlabFilteredRepos : githubFilteredRepos;
  const activeEmptyMessage = isGitlabView
    ? hasGitlabLinked
      ? "Aucun dépôt GitLab trouvé."
      : "Connecte ton compte GitLab pour afficher ses dépôts."
    : hasGithubLinked
      ? "Aucun dépôt GitHub trouvé."
      : "Connecte ton compte GitHub pour afficher ses dépôts.";

  const isRepoPrivate = (repo: GitRepoDto) =>
    Boolean(repo.private ?? repo.isPrivate);
  const canScanRepo = (repo: GitRepoDto) =>
    repo.provider === "GITLAB" || !isRepoPrivate(repo);
  const findAssignedRepo = (repo: GitRepoDto) =>
    scannedRepositories.find((entry) => entry.repoUrl === repo.htmlUrl);
  const selectedRepoEntry = selectedRepo
    ? gitRepos.find((repo) => repo.fullName === selectedRepo)
    : undefined;
  const selectedAssignedRepo = selectedRepoEntry
    ? findAssignedRepo(selectedRepoEntry)
    : undefined;
  ...
  const tabButtonClass = (tab: "github" | "gitlab" | "url") => {
    const base =
      "flex items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-headline font-semibold border transition-all duration-200";
    if (scanSource !== tab) {
  ...
  const handleStartScan = async () => {
    ...
    try {
      const { data } = await startScan({
        ...
        containerPort: isDockerImage
          ? parseInt(containerPort, 10) || 3000
          : undefined,
        // Idée 3 — OS cible
      ...
      setLogs((prev) => [
        ...prev,
        parseLogLine("[ERROR] Failed to start scan. Is the backend running?"),
      ]);
      setScanning(false);
    }
  };
  ...
  const renderRepoCard = (repo: GitRepoDto) => {
    ...
    return (
      ...
      >
        ...
        {repo.description && (
          <p className="text-xs text-outline-variant mb-2 line-clamp-1">
            {repo.description}
          </p>
        )}
        ...
        <div className="flex items-center gap-3 text-xs text-outline">
          {repo.language && (
            <span className="flex items-center gap-1">
              <span
                ...
                style={{
                  backgroundColor: langColor[repo.language] || "#8b949e",
                }}
              ></span>
              {repo.language}
            </span>
          ...
          <span
            className={`ml-auto px-1.5 py-0.5 rounded text-[10px] font-medium ${isRepoPrivate(repo) ? "bg-error/10 text-error" : "bg-tertiary/10 text-tertiary"}`}
          >
            {isRepoPrivate(repo) ? "Privé" : "Public"}
          </span>
        </div>
        {/* ── Badge planification active ── */}
        {(() => {
          const repoEntry = scannedRepositories.find(
            (r) => r.repoUrl === repo.htmlUrl,
          );
          const sched = repoEntry
            ? scheduledSummary[String(repoEntry.id)]
            : undefined;
          if (
            !sched ||
            (sched.status !== "ACTIVE" &&
            ...
            return null;
          return (
            <div className="mt-2 flex items-center gap-1.5 px-2 py-1 rounded-lg bg-violet-500/10 border border-violet-500/20">
        ...
        <div className="mt-2 flex gap-2">
          <button
            onClick={(e) => {
              e.stopPropagation();
              handleQuickScan(repo);
            }}
            disabled={
              (scanning && selectedRepo === repo.fullName) || !canScanRepo(repo)
            }
            title={
              !canScanRepo(repo)
                ? "Les dépôts privés GitHub ne peuvent pas être scannés ici"
                : undefined
            }
          ...
          >
            <span className="material-symbols-outlined text-sm">
              {!canScanRepo(repo)
                ? "lock"
                : scanning && selectedRepo === repo.fullName
                  ? "progress_activity"
                  : "radar"}
            </span>
            {!canScanRepo(repo)
          ...
          {/* Planifier button */}
          <button
            onClick={(e) => handleOpenScheduleModal(e, repo)}
            title="Planifier un scan automatique"
            className="flex items-center justify-center px-3 py-2 rounded-lg text-xs border border-violet-500/25 text-violet-400 hover:bg-violet-500/10 hover:border-violet-500/50 transition-all duration-200"
          >
            <span className="material-symbols-outlined text-sm">
              calendar_clock
            </span>
    ...
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-12rem)] relative overflow-hidden">
      ...
      {user && (
        <div className="z-10 mb-8">
          <div
            className={`grid gap-2 rounded-3xl border border-outline-variant/[0.15] bg-surface-container-low p-2 glass-panel shadow-lg ${hasGithubLinked && hasGitlabLinked ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}
          >
            {hasGithubLinked && (
              <button
      ...
      {user &&
        scanSource !== "url" &&
        ((scanSource === "github" && hasGithubLinked) ||
          (scanSource === "gitlab" && hasGitlabLinked)) && (
                ...
                {!reposLoading && !reposError && activeRepos.length > 0 && (
                  <section
                    className={`rounded-2xl border p-4 ${
                      isGitlabView
                        ? "border-orange-400/15 bg-orange-400/[0.03]"
              ...
              {selectedRepo && selectedRepoMatchesView && (
                <div className="px-6 py-3 bg-primary/5 border-t border-primary/20 flex items-center justify-between">
                  <div className="flex items-center gap-2 text-sm text-primary">
                    <span className="material-symbols-outlined text-sm">
                      {scanning ? "progress_activity" : "check_circle"}
                    </span>
                    <div className="flex flex-col gap-1">
                      ...
                      {selectedAssignedRepo?.clientNames?.length ? (
                        <div className="flex flex-wrap gap-1.5">
                          {selectedAssignedRepo.clientNames.map(
                            (clientName) => (
                              <span
                    ...
                    <span className="text-outline text-xs">
                      {scanning ? "— scan en cours..." : "— sélectionné"}
                    </span>
                  </div>
                  <button
                    onClick={() => {
      ...
      {(!user || scanSource === "url") && (
        ...
        >
          ...
          <div className="relative flex items-center justify-center h-64 w-64">
            <div
              className={`absolute w-full h-full rounded-full ${scanning ? "scan-ring-outer" : "border-2 border-primary/20"}`}
            ></div>
            <div
              className={`absolute w-4/5 h-4/5 rounded-full ${scanning ? "scan-ring-inner" : "border-2 border-dashed border-secondary/20"}`}
            ></div>
          ...
          <div className="w-full text-center space-y-8">
            ...
            <div className="relative max-w-2xl mx-auto group">
              <div
                className={`absolute inset-0 blur-xl opacity-0 group-focus-within:opacity-100 transition-opacity duration-500 rounded-full ${
                  scanType === "dast"
                    ? "bg-orange-500/20"
                    : scanType === "docker-image"
                      ? "bg-purple-500/20"
              ...
              ></div>
              <div
                className={`relative flex items-center p-2 bg-surface-container-low rounded-full border glass-panel transition-all shadow-2xl ${
                  scanType === "dast"
                    ? "border-orange-500/40 group-focus-within:border-orange-500/70"
                    : scanType === "docker-image"
                      ? "border-purple-500/40 group-focus-within:border-purple-500/70"
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    scanType === "dast"
                      ? setDastTargetUrl(e.target.value)
                      : scanType === "docker-image"
                        ? setDockerImageRef(e.target.value)
            ...
            {scanType === "docker-image" && (
              <div className="flex items-center gap-3 max-w-xs mx-auto -mt-4">
                <label className="text-xs text-outline whitespace-nowrap">
                  Port exposé :
                </label>
                <input
            ...
            <div className="flex items-center justify-center gap-4 flex-wrap">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span
                    className={`material-symbols-outlined text-outline text-lg transition-transform duration-200 ${dropdownOpen ? "rotate-180" : ""}`}
                  >
                    expand_more
                  </span>
                </button>
                  ...
                  <>
                    ...
                    <div className="absolute z-20 mt-2 w-full rounded-2xl border border-outline-variant/[0.15] bg-surface-container-low glass-panel shadow-2xl shadow-primary/5 max-h-60 overflow-y-auto custom-scrollbar">
                      {scanTypes.map((type) => (
                        ...
                        >
                          <span
                            className={`material-symbols-outlined text-lg ${scanType === type.value ? "text-primary" : "text-outline"}`}
                          >
                            {type.icon}
                          </span>
              ...
              >
                ...
                <span
                  className={`material-symbols-outlined text-sm transition-transform ${showAdvanced ? "rotate-180" : ""}`}
                >
                  expand_more
                </span>
              </button>
            ...
            {scanType === "dast" && (
              <p className="text-[11px] text-orange-400/70 text-center -mt-4">
                ZAP scannera cette URL en direct — l'application doit être
                accessible et en cours d'exécution
              </p>
            )}
            {scanType === "docker-image" && (
              <p className="text-[11px] text-purple-400/70 text-center -mt-4">
                L'image sera téléchargée, analysée (Trivy + Grype), démarrée sur
                le port indiqué, puis scannée avec ZAP
              </p>
            )}

            <div className="flex items-center justify-center gap-8 mt-4">
              <div className="flex items-center gap-2 text-xs text-outline">
                <span className="w-2 h-2 rounded-full bg-tertiary"></span> SAST
                Analysis
              </div>
              <div className="flex items-center gap-2 text-xs text-outline">
        ...
        >
          ...
          >
            ...
            <div className="p-5 space-y-5 max-h-[75vh] overflow-y-auto custom-scrollbar">
              {/* Warning if repo not scanned yet */}
              {!scannedRepositories.find(
                (r) => r.repoUrl === scheduleModalRepo.htmlUrl,
              ) && (
                <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg bg-amber-500/10 border border-amber-500/20 text-xs text-amber-400">
                  <span className="material-symbols-outlined text-sm shrink-0 mt-0.5">
                    warning
                  </span>
                  <span>
                    Ce repository doit être scanné au moins une fois avant de
                    planifier.
                  </span>
                </div>
              )}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        type="date"
                        className="w-full bg-surface-container-low border border-outline-variant/[0.2] rounded-lg px-3 py-2 text-sm text-on-surface focus:ring-1 focus:ring-violet-500 focus:border-violet-500/50 appearance-none cursor-pointer [color-scheme:dark]"
                        value={scheduleDate}
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="relative flex-1">
                        <select
                          className="w-full bg-surface-container-low border border-outline-variant/[0.2] rounded-lg px-3 py-2 pr-7 text-sm text-on-surface focus:ring-1 focus:ring-violet-500 focus:border-violet-500/50 appearance-none cursor-pointer [color-scheme:dark]"
                          value={scheduleHour}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="text"
                      className="w-full bg-surface-container-low border border-outline-variant/[0.2] rounded-lg px-3 py-2 text-sm text-on-surface placeholder:text-outline focus:ring-1 focus:ring-violet-500 focus:border-violet-500/50"
                      placeholder="main"
                ...
                {/* Target domain (optionnel) */}
                <div>
                  <label className="block text-[10px] text-outline mb-1">
                    Target domain{" "}
                    <span className="text-outline/50">(optionnel)</span>
                  </label>
                  <input
                    type="text"
                    className="w-full bg-surface-container-low border border-outline-variant/[0.2] rounded-lg px-3 py-2 text-sm text-on-surface placeholder:text-outline focus:ring-1 focus:ring-violet-500 focus:border-violet-500/50"
                    placeholder="example.com"
                    value={scheduleTargetDomain}
                    onChange={(e) => setScheduleTargetDomain(e.target.value)}
                  />
                </div>
                    ...
                    >
                      <div className="space-y-0.5 min-w-0">
                        ...
                        <p className="text-[10px] text-outline/60">
                          {sched.runCount} exécution(s)
                        </p>
                      </div>
                      <div className="flex gap-1 shrink-0">
                        {sched.status === "ACTIVE" && (
                          <button
                            onClick={() => handlePauseSchedule(sched.id)}
                            title="Pause"
                        ...
                        {sched.status === "PAUSED" && (
                          <button
                            onClick={() => handleResumeSchedule(sched.id)}
                            title="Reprendre"
