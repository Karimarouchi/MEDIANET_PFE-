...
const Layout: React.FC = () => {
  ...
  const navItems = [
    ...
    { path: '/repositories', label: 'Repositories', icon: 'code_blocks', permission: 'REPOSITORIES' },
    { path: '/scans', label: 'Scans', icon: 'radar', permission: 'SCANS' },
    { path: '/ssl-analysis', label: 'SSL Analysis', icon: 'verified_user', permission: 'SSL_ANALYSIS' },
    { path: '/server-config', label: 'Server Config', icon: 'terminal', permission: 'SERVER_CONFIG' },
    { path: '/pipeline', label: 'Pipeline', icon: 'settings_ethernet', permission: 'PIPELINE' },
  ].filter((item) => hasPermission(item.permission));

  const showAdmin = hasAnyPermission(adminPermissionKeys) && user?.systemRole === 'ADMIN';
      ...
      <aside className="fixed left-0 top-0 h-screen w-64 border-r border-outline-variant/[0.15] bg-surface-container-low hidden md:flex flex-col py-6 z-50">
        <div className="px-6 mb-10 flex items-center gap-3">
          <div className="w-8 h-8 bg-gradient-to-br from-primary to-secondary rounded-lg flex items-center justify-center">
            <span className="material-symbols-outlined text-on-primary text-xl" style={{ fontVariationSettings: "'FILL' 1" }}>shield</span>
          </div>
          <div>
            <h2 className="font-headline font-bold text-on-surface text-base tracking-tight leading-none">Tactical OS</h2>
            <p className="text-[10px] text-outline uppercase tracking-widest mt-1">Quantum Observer v1.0</p>
          </div>
        </div>
          ...
          {user ? (
            <div className="flex items-center gap-3 px-4 py-3 rounded-lg bg-surface-container mt-1">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-headline font-semibold text-on-surface truncate">{user.login}</p>
                <p className="text-[10px] text-outline truncate">{user.role} · {user.primaryProvider}</p>
              </div>
              <button
                onClick={logout}
                title="Déconnexion"
