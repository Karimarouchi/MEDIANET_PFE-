...
public class DeployService {
    ...
    public DeployRunDto deploy(Long serverId, boolean force, DeployRun.TriggerType triggerType) {
        ...
        DeployRun run = DeployRun.builder()
                ...
                .build();
        run = deployRunRepo.save(run);

        if (blocked && !force) {
            return toDto(run, gate.verdict.blocking());
        }

        String command = buildDeployCommand(path, branch);
        StringBuilder liveLog = new StringBuilder(run.getLog() == null ? "" : run.getLog());
        if (blocked) {
        ...
        try {
            ...
            run.setLog(trimLog(liveLog.toString()));
            run.setFinishedAt(LocalDateTime.now());
            if (result.exitCode() == 0) {
                run.setStatus(DeployRun.Status.SUCCESS);
                liveLog.append("Déploiement terminé (exit 0).\n");
                run.setLog(trimLog(liveLog.toString()));
            } else {
                run.setStatus(DeployRun.Status.FAILED);
                liveLog.append("Échec SSH / commande (exit ").append(result.exitCode()).append(").\n");
                run.setLog(trimLog(liveLog.toString()));
            }
        } catch (Exception e) {
            run.setStatus(DeployRun.Status.FAILED);
            liveLog.append("Erreur : ").append(e.getMessage() != null ? e.getMessage() : e.getClass().getSimpleName());
            run.setLog(trimLog(liveLog.toString()));
            run.setFinishedAt(LocalDateTime.now());
            log.warn("[Deploy] SSH failed server={} : {}", serverId, e.getMessage());
        }
        return toDto(deployRunRepo.save(run), gate.verdict.blocking());
    }
